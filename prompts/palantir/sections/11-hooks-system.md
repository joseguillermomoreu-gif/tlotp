...
**DESPUÉS del resumen**, analizar la configuración contra la documentación oficial y aportar conclusiones:

#### 3.1: Detectar conflictos

Buscar hooks que puedan interferir entre niveles:
- Mismo evento + mismo matcher en global Y proyecto → posible conflicto
- Informar: "El hook X en global puede interferir con el hook Y en proyecto"

#### 3.2: Detectar oportunidades

Comparar hooks existentes con los patrones recomendados de la documentación:

**Patrones recomendados** (verificar contra docs live):
- Notification hook para alertas → ¿El usuario tiene uno?
- PostToolUse con formatter → ¿Hay auto-format configurado?
- PreToolUse para proteger archivos sensibles → ¿Hay protección de .env, secrets?
- SessionStart con compact → ¿Hay re-inyección de contexto?
...
✅ Buenas prácticas detectadas:
   • Tienes auto-format con Prettier tras ediciones (👍)
   • Validación de comandos Bash antes de ejecutar (👍)

⚠️ Oportunidades de mejora:
   1. No tienes hook de Notification → Podrías recibir alertas
      cuando Claude termine o necesite tu input
   2. No tienes protección de archivos sensibles → Un PreToolUse
      podría bloquear ediciones a .env, secrets, etc.
   3. No tienes re-inyección post-compact → Si Claude compacta
      contexto, pierde instrucciones críticas

¿Quieres crear alguno de estos hooks? (pasa a opción "Crear nuevo hook")
```

#### 3.3: Mostrar detalle de cada hook

Para cada hook existente, mostrar detalle completo:
```
🪝 Hook: PostToolUse > Edit|Write
   Tipo: command
   Comando: jq -r '.tool_input.file_path' | xargs npx prettier --write
   Scope: Global
   Propósito: Auto-formatear archivos editados con Prettier
...
### Paso 2: Determinar configuración (nosotros lo calculamos)

**Según lo que el usuario quiera automatizar**, consultar la documentación oficial obtenida via WebFetch y determinar:

#### Tabla de decisiones

| Lo que quiere el usuario | Evento | Tipo | Scope recomendado |
|--------------------------|--------|------|-------------------|
| Recibir notificaciones | `Notification` | `command` | Global (aplica a todo) |
| Formatear tras edición | `PostToolUse` (matcher: `Edit\|Write`) | `command` | Proyecto (cada proyecto tiene su formatter) |
| Bloquear archivos sensibles | `PreToolUse` (matcher: `Edit\|Write`) | `command` | Proyecto (cada proyecto tiene sus archivos sensibles) |
| Validar comandos peligrosos | `PreToolUse` (matcher: `Bash`) | `command` | Global (seguridad universal) |
| Re-inyectar contexto tras compact | `SessionStart` (matcher: `compact`) | `command` | Proyecto (cada proyecto tiene su contexto) |
...
**Si el usuario elige "Otra cosa"**: Analizar su descripción contra la tabla de eventos de la documentación oficial y recomendar la combinación adecuada.
...
### Paso 5: Generar configuración JSON

**Generar el JSON completo** listo para insertar:
